...
public class ComponentUtils {
     ...
     * @return
     */
    public static String getInitValue(Component component, String params) {
        if(component.getInitValue() == null) {
            return getInitValue(component.getInitMethod(), params);
        }
     ...
     * @return
     */
    public static String getInitValue(String methodStr, String params) {
        if(StringUtils.isNotBlank(methodStr)) {
            try {
                ...
                Method method;
                Object o = null;
                if("com.thinkgem.jeesite.modules.form.util.InitUtils.getUsername".equals(methodStr)) {
                    method = obj.getClass().getMethod(methodStr.substring(index + 1), String.class);
                    o = method.invoke(null, params);
                } else {
                    method = obj.getClass().getMethod(methodStr.substring(index + 1));
    ...
    public static String initComponent(Map<String,Object> columnMap,boolean initValue) {
        ...
        if(columnMap != null) {
            ...
            params = (String) columnMap.get("params");
        }
        if(StringUtils.isNotBlank(value) && StringUtils.isNotBlank(columnName)) {
            ...
            if(component != null) {
                ...
                if(chargeMoreData(value)) {
                    String defalutValue = initValue ? getInitValue(component, params) : null;
                    content = handlerMoreDateComponent(value, content, (List<Dict>) columnMap.get("optData"), defalutValue);
                } else {
                    content = content.replace("${colValue}",initValue ? getInitValue(component, params) : ("${" + columnName + "}"));
                }
