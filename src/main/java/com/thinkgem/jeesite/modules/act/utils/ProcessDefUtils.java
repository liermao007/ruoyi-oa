...
public abstract class ProcessDefUtils {
    ...
    public static List<Object[]> processList(String category) {
        List<Role> roles= UserUtils.getRole();
        User user=UserUtils.getUserId();
      /*  for(Role role:roles) {
            if (StringUtils.equalsIgnoreCase(role.getName(), "集团管理员") || StringUtils.equalsIgnoreCase(UserUtils.getUser().getCompany().getId(), user.getCompanyId())) {
                category = "1";
            } else if (StringUtils.equalsIgnoreCase(UserUtils.getUser().getId(), "1")) {
                category = "";
            } else {
                category = "2";
            }
