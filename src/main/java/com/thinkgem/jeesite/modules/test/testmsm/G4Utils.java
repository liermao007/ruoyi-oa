...
public class G4Utils {
    ...
    public static String encryptBasedDes(String data) {
        ...
        try {
            ...
            encryptedData = Base64.encode(cipher.doFinal(data.getBytes()));
        } catch (Exception e) {
    ...
    public static boolean Translate(String value, boolean def) {
        if (value == null) {
            return def;
        } else {
            value = value.trim();
     ...
     * @return 替换后的字符串
     */
    public static String replaceString(String str, int start, String des) {
        int endIndex;
        String startString;

        if (des == null || des.length() == 0) return str;
        if (str == null) return des;
        if (str.length() == 0) return des;
        if (start < 0) start = 0;
        if (start >= str.length()) return str + des;
        startString = str.substring(0, start);
        endIndex = start + des.length();
        if (endIndex >= str.length()) return startString + des;
        return startString + des + str.substring(endIndex);
    }
