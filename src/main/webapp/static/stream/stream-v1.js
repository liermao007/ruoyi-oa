	...
	StreamUploader.prototype = {
		...
		initializer: function(file){
			...
			if (b && StreamUploader.canUpload()) {
				...
				this.set("name", new Date().getTime()+"_"+(b.RelativePath || b.webkitRelativePath || b.name || b.fileName));
	...
	Main.prototype = {
		...
		addStreamTask : function(a) {
			...
			if (!this.config.customered) {
                ...
                parent.document.getElementsByName('FJMC')[0].value=parent.document.getElementsByName('FJMC')[0].value+aa+",";
				this.getNode("stream-file-name", cell_file).getElementsByTagName("strong")[0].innerHTML =aa;
