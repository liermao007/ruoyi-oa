...
<head>
	...
	<script type="text/javascript">
		var zrFlag = '${zrFlag}';
		var key, lastValue = "", nodeList = [], type = getQueryString("type", "${url}"), url = "";
        url = getQueryString("child", "${url}") == 'role' ? "${ctx}/sys/role/treeData?checkAll="+"${checkAll}" : "${ctx}/sys/user/treeData?checkAll="+"${checkAll}";
		var tree, setting = {view:{selectedMulti:false,dblClickExpand:false},check:{enable:"${checked}",nocheckInherit:true},
				async:{enable:(type==3),url:url,autoParam:["id=officeId"]},
		...
		if(chkStyle) {
			setting.check.chkStyle = chkStyle;
		...
		function searchNode() {
			...
			hideAllNode(nodes);
			nodeList = tree.getNodesByFilter(filterFunc);
		...
		function filterFunc(node) {
			...
			if(zrFlag == 1) {
				let zr = $('#zr:checked').val();
				zrSelect = !zr || (zr && node.grade == '主任')
			}

			return node.name.indexOf($('#key').val()) > -1 && zrSelect
		}
	</script>
</head>
<body>
	...
	<div id="search" class="form-search hide" style="padding:10px 0 0 13px;">
		<c:if test="${zrFlag == 1}">
			<label class="control-label" style="padding:5px 5px 3px 0;">主&nbsp;&nbsp;&nbsp;&nbsp;任：</label>
			<input type="checkbox" value="1" id="zr" onclick="changeZr()"/>
			<br>
		</c:if>
