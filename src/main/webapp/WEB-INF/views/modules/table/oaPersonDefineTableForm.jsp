...
<head>
	...
	<script type="text/javascript">
		...
		function addRow(list, idx, tpl, row){
            ...
            if(!row.tableStatus) row.tableStatus = 500
        ...
        function changeColumnType(id, s){
            var v = $('#' + id).val()
            if(v != 'VARCHAR' && v != 'REMARK') {
                $('#' + s).val('')
                $('#' + s).attr('disabled',true)
                $('#' + s).removeClass('required numeric')
            } else {
                if($('#' + s).val() == '') $('#' + s).val(100)
                $('#' + s).removeAttr('disabled')
                $('#' + s).addClass('required numeric')
            }
        }
    ...
    <form:form id="inputForm" modelAttribute="oaPersonDefineTable" action="${ctx}/table/oaPersonDefineTable/save" method="post" class="form-horizontal">
        ...
        <fieldset>
            ...
            <div class="control-group">
                ...
                <div class="controls">
                    <select id="tableProperty" name="tableProperty" class="required input-medium">
                        <c:forEach items="${fns:getDictList('act_type')}" var="dict">
