...
<ul class="breadcrumb">
    ...
    <ul style="list-style: none; width: 90%;text-align: center">
           <c:forEach items="${page.list}" var="oaPersonDefineTable">
            <li id="liStyle" >
                    <a style="font-size: 16px;" href="${ctx}/oa/flow/myFlow?procDefId=${oaPersonDefineTable.procDefId}&type=0">
        ...
        <c:if test="${fns:findCustomProcess().size()>0}">
            <li id="liStyle">
                <a style="font-size: 16px;" href="${ctx}/process/customProcess/form">
                    自定义流程
                </a>
            </li>
        </c:if>
        <c:if test="${empty page.list}">
            <div class="daiban_Con11">
                <div>
                    ...
                    <span><font size="5" >您暂时还没有待办流程，休息一下吧！</font></span>
                </div>
            </div>
        </c:if>
    </ul>
