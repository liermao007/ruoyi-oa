...
<body>
    <ul class="breadcrumb">
        <input id="btnCancel" class="btn" style="margin: 1px" type="button" value="返 回" onclick="history.go(-1)"/>
    </ul>
	<form:form id="inputForm" modelAttribute="oaNotify" action="${ctx}/oa/oaNotify/save" method="post" cssStyle="margin: 20px">
		<form:hidden path="id"/>
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:textarea path="content" htmlEscape="false" rows="6" cssStyle="width: 270px"/>
				<span class="help-inline"><font color="red">*</font> </span>
			</div>
		</div>
			...
			<div class="control-group">
				...
				<div class="controls">
	                <sys:treeselect id="oaNotifyRecord" name="oaNotifyRecordIds" value="${oaNotify.oaNotifyRecordIds}" labelName="oaNotifyRecordNames" labelValue="${oaNotify.oaNotifyRecordNames}"
						title="用户" url="/sys/office/treeData?type=3" notAllowSelectParent="true" checked="true"/>
					<span class="help-inline"><font color="red">*</font> </span>
				</div>
			</div>
		</c:if>
		...
		<div class="form-actions" align="center">
			<c:if test="${oaNotify.status ne '1'}">
				<shiro:hasPermission name="oa:oaNotify:edit"><input id="btnSubmit" class="btn btn-primary" style="width: 240px" type="submit" value="保 存"/>&nbsp;</shiro:hasPermission>
			</c:if>
			<input  class="btn" type="button" value="返 回" style="width: 240px" onclick="self.location=document.referrer;"/>
		</div>
	</form:form>
</body>
</html>