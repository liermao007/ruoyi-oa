...
<%@ attribute name="zrFlag" type="java.lang.String" required="false" description="是否过滤主任"%>
<%@ attribute name="smallBtn" type="java.lang.Boolean" required="false" description="缩小按钮显示"%>
<%@ attribute name="hideBtn" type="java.lang.Boolean" required="false" description="是否显示按钮"%>
<%@ attribute name="disabled" type="java.lang.String" required="false" description="是否限制选择，如果限制，设置为disabled"%>
<%@ attribute name="dataMsgRequired" type="java.lang.String" required="false" description=""%>
<div class="input-append">
	<%--<form id="agentidFrom" action="" method="post"></form>--%>
...
<script type="text/javascript">
	$("#${id}Button, #${id}Name").click(function(){
		...
		// 正常打开	
		top.$.jBox.open("iframe:${ctx}/tag/treeselect?url="+encodeURIComponent("${url}")+"&module=${module}&checked=${checked}&zrFlag=${zrFlag}&extId=${extId}&isAll=${isAll}", "选择${title}", 300, 420, {
			ajaxData:{selectIds: $("#${id}Id").val()},buttons:{"确定":"ok", ${allowClear?"\"清除\":\"clear\", ":""}"关闭":true}, submit:function(v, h, f){
				if (v=="ok"){
                    ...
                    if("${id}" == "agentid"){
                        $("#agentidFrom").attr("action","${ctx}/test1/oaRunProcess/selectCheck");
                        $("#agentidFrom").submit();
                    }else{
					}
                ...
                if(typeof myCallBack == 'function'){
                    myCallBack($("#${id}Name").val(),$("#${id}Id").val(),$("#${id}Pid").val());
                };
